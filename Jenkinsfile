...
pipeline {
    ...
    // }
    environment {
        PROJECT = "WELCOME TO DEVOPS B40 BATCH - Jenkins Class"
    ...
    stages {
        ...
        stage('Dev Sonarqube SAST') {
            ...
            agent { label 'DEV' }
            steps { 
                withSonarQubeEnv('devopsb40-SonarQube-DEV'){
                     sh "mvn clean verify sonar:sonar \
                     -Dsonar.projectKey=spring-boot-app-dev \
                     -Dsonar.projectName=spring-boot-app-dev \
                     -Dsonar.host.url=http://sonar.k8sgk989.xyz/:9000"
                }
        ...
        stage('Dev docker build') {
            ...
            agent { label 'DEV' }
            steps { 
                sh "sudo docker build -t sreeharshav/devopsb40spring:$BUILD_NUMBER ."
            }
        }
        ...
        stage('Publish Trivy Report') {
            ...
            agent { label 'MASTER' }
            steps {
                        sh 'aws s3 cp s3://sreeterraformbucket/dev_trivy_report.html dev_trivy_report.html'
        ...
        stage('Dev Deploy Docker Image') {
            ...
            steps { 
                ...
                sh "sudo docker run --rm -dit --name springbootapp -p 8080:8080 sreeharshav/devopsb40spring:$BUILD_NUMBER"
            }
        }
        stage('Dev Validate Deployment') {
            ...
            agent { label 'DEV' }
            steps { 
                sh "sleep 30 && curl http://dev.k8sgk989.xyz:8080 || exit 1"
            }
        }
        stage ('Dev DAST') {
            when {
          ...
          agent { label 'DEV' }  
          steps {
             sh 'sudo docker run -t owasp/zap2docker-stable zap-baseline.py -t http://dev.k8sgk989.xyz:8080 || true'
            }
        }
//-----------------------------PRODUCTION---------------
...
//             agent { label 'PROD' }
//             steps { 
//                 withSonarQubeEnv('devopsb40-SonarQube-PROD'){
//                      sh "mvn clean verify sonar:sonar \
//                      -Dsonar.projectKey=spring-boot-app-prod \
//                      -Dsonar.projectName=spring-boot-app-prod \
//                      -Dsonar.host.url=http://sonar.azureb42.xyz:9000"
//                 }
...
//             agent { label 'PROD' }
//             steps { 
//                 sh "sudo docker build -t sreeharshav/devopsb40springprod:$BUILD_NUMBER ."
//             }
//         }
...
//                         sh """sudo trivy image sreeharshav/devopsb40springprod:$BUILD_NUMBER --security-checks vuln --exit-code 0 --severity CRITICAL --timeout 15m --format template --template \"@html.tpl\" --output reports/prod_trivy_report.html  """
//                         sh 'aws s3 cp reports/prod_trivy_report.html s3://sreeterraformbucket/prod_trivy_report.html'
//                     }
//                 }
...
//             agent { label 'MASTER' }
//             steps {
//                         sh 'aws s3 cp s3://sreeterraformbucket/prod_trivy_report.html prod_trivy_report.html'
...
//             steps { 
//                 sh "sudo docker stop springbootapp || sudo docker ps"
//                 sh "sudo docker run --rm -dit --name springbootapp -p 8080:8080 sreeharshav/devopsb40springprod:$BUILD_NUMBER"
//             }
//         }
...
//             agent { label 'PROD' }
//             steps { 
//                 sh "sleep 30 && curl http://prod.awsb59.xyz:8080 || exit 1"
//             }
//         }
//         stage ('PROD DAST') {
//             when {
...
//           agent { label 'PROD' }  
//           steps {
//              sh 'sudo docker run -t owasp/zap2docker-stable zap-baseline.py -t http://prod.awsb59.xyz:8080 || true'
//             }
//         }
//     }
//     post {
